...
//builds a variable to add all CSS rules into
//This is what we spit out as the basis of our css file
$text = ''; 
...
//used to add a custom font from Google Fonts API    
...
$text = $font."body {font-family: '".$font_name."', ".$font_style." !important;}";
    
}    

//Sets colors for all a, and a:hover text globally
if (isset($_POST['link_color']) && $_POST['link_color'] != '#0088CC') {
    
    $text .= 'a, a:hover {color: '.$_POST['link_color'].';}';
    
}
    
//Modifies header to be flat. Not very clean, but works
if (isset($_POST['header_color']) && $_POST['header_color'] != '#ffffff') {
    
    $text .= '.navbar-inverse .navbar-inner {
...
//Color for the text, not the background, for dropdown menu
...
//title element of normal slider (not tested with full page slider)
...
//carousel descriptiion text color in normal slider (not tested with full page slider)
...
//Sets normal slider background (not tested with full page slider)
if (isset($_POST['carousel_metadata_background_color'])) {
    
    $bg_color = $_POST['carousel_metadata_background_color'];
    
    //function to return an RGBa from Hex (need to support opacity)
...
//set our variable to define the value   
$rgba = "rgba(".$rgba_val.", 0.75)";

}

//overrides the above if not using the default opacity of 0.75 (this is not clean)
...
//Changes the colors of the carousel arrow colors
...
//changes the border colors of the arrows
...
//modifies the opacity of the arrow backgrounds
...
//Display the title/duration/views below the thumbnails, not on top of them
...
//hides mediatype icon    
if (isset($_POST['show_mediatype_icon']) && $_POST['show_mediatype_icon'] == 0) {
...
//Sets the color of pills in the KMS instance (think search results page)
...
//sets the color of tags text underneath the player    
...
//sets the category pills colors under the video player
...
//add some extras I feel improve the UI, not configured by customer
$text.=' #mediaContainer .entryTitle {
...
//Limits the number of thumbnails that should show up in the home page results
//best when used in conjunction with thumbnail resizer
...
//makes the header stick to the top of the KMS
//Deprecated - there's now a custom module for this in PS custom modules repository
...
//makes thumbnails black and white
...
//overlay a small icon on top of your thumbnail (such as your logo)
//takes a hosted image as an input
//IMPORTANT! Width and Height must be 37px x 37px (cause I couldn't make it work otherwise)
...
//this implements the resizing of thumbnails on home page
//checks if a user has elected that change, then sets the values defined on lines 27-73
if ($default != 'true') {
...
//Set headers to make this file download
header("Cache-Control: public");
header("Content-Description: File Transfer");
//set title for the filename and add _custom.css so it has a file type
header("Content-Disposition: attachment; filename=".$title."_custom.css");
header("Content-Type: text/xml");
header("Content-Transfer-Encoding: binary");

//download all CSS vars to new css file name $title_custom.css   
echo $text; 
